...
// import { createSale as searchCustomer } from '../../../http/saleApi';
...
const Creditbooks = ({
  ...
  const handleCustomerSearch = e => {
    ...
    searchCustomer(e.target.value)
      .then(handleCustomerSearchSucuess)
      .catch(handleCustomerSearchErr);
  };

  const handleCustomerSearchSucuess = () => {
    fetchApi(false);
    setCartItems([]);
    setRevdAmount(parseFloat(0).toFixed(2));
    setFetchApiInfo({ type: 'success', message: 'Bill created succuess' });
    ...
    <Fragment>
      ...
      {openConfirm && (
        <ConfirmationPopup
          ...
          content='Are you sure want to discard this credit and create a new credit?'
        />
      )}
    </Fragment>
  );
